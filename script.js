document.addEventListener("DOMContentLoaded", () => {
    ...
    const GAS_WEBAPP_URL = "https://script.google.com/macros/s/AKfycbyo0ZD8FsQDKdT06MqHZFP9Cbb8V3vL2XBtci5pOMvxoVKVGDs5waaseme_5JJ3YjUN/exec";

    // --- 🐛 芋虫 ---
    let wormClicks = 0; let wormCrushed = false;
    ...
    worm.addEventListener("click", () => {
        ...
        wormClicks++;
        if (wormClicks >= 30) {
            wormCrushed = true; worm.textContent = "💥"; worm.classList.add("crushed");
            bubble.textContent = "論理が……崩壊……"; setTimeout(() => bubble.classList.add("hidden"), 2000); return;
        }
        bubble.textContent = wormLines[Math.floor(Math.random() * wormLines.length)];
        bubble.classList.remove("hidden");
        clearTimeout(worm.bubbleTimer); worm.bubbleTimer = setTimeout(() => bubble.classList.add("hidden"), 2000);
    });

    // --- 🔴 タイトル長押し ---
    let holdTime = 0, startTime = 0;
    ...
    const startHold = (e) => {
        ...
        holdInterval = setInterval(() => {
            holdTime += 50; inkBg.style.width = `${holdTime * 0.6}px`; inkBg.style.height = `${holdTime * 0.6}px`;
        }, 50);
    };
    const endHold = () => {
        ...
        } else { holdTime = 0; inkBg.style.width = "0px"; inkBg.style.height = "0px"; }
    };
    startBtn.addEventListener("mousedown", startHold); startBtn.addEventListener("mouseup", endHold);
    startBtn.addEventListener("mouseleave", endHold);
    startBtn.addEventListener("touchstart", startHold, {passive: false}); startBtn.addEventListener("touchend", endHold);
    ...
    questionsData.forEach(q => {
        ...
        if (q.type === "slider") {
            div.innerHTML = `<label>${q.text}</label>
                ...
                <span>${q.left}</span><span>${q.right}</span></div><input type="range" id="${q.id}" min="0" max="100" value="50">`;
            sContainer.appendChild(div);
        } else {
            let html = `<label>${q.text}</label><div class="radio-options">`;
            q.options.forEach((opt, i) => { html += `<label><input type="radio" name="${q.id}" value="${opt.val}" ${i===0?'checked':''}> ${opt.label}</label>`; });
            html += `</div>`; div.innerHTML = html; rContainer.appendChild(div);
        }
    });
    ...
    motifs.forEach(m => {
        const btn = document.createElement("button"); btn.className = "motif-btn"; btn.textContent = m.label;
        btn.onclick = () => {
            const idx = selectedMotifs.findIndex(x => x.id === m.id);
            if (idx > -1) selectedMotifs.splice(idx, 1);
            else if (selectedMotifs.length < 3) selectedMotifs.push(m);
            document.querySelectorAll(".motif-btn").forEach(b => {
                b.classList.remove("selected"); const mObj = motifs.find(x => x.label === b.textContent);
    ...
    let isDrawing = false;
    ctx.lineWidth = 20; ctx.lineCap = "round"; ctx.strokeStyle = "#a1c4fd";
    const getPos = (e) => {
    ...
    const drawStart = (e) => { e.preventDefault(); isDrawing = true; const p = getPos(e); ctx.beginPath(); ctx.moveTo(p.x, p.y); };
    const drawMove = (e) => { if(!isDrawing) return; e.preventDefault(); const p = getPos(e); ctx.lineTo(p.x, p.y); ctx.stroke(); };
    const drawEnd = () => { isDrawing = false; };
    canvas.addEventListener("mousedown", drawStart); canvas.addEventListener("mousemove", drawMove); canvas.addEventListener("mouseup", drawEnd); canvas.addEventListener("mouseleave", drawEnd);
    ...
    let isDragging = false; let finalPos = { x: 50, y: 50 };
    const moveParticle = (clientX, clientY) => {
        const rect = dragArea.getBoundingClientRect();
        let nx = clientX - rect.left; let ny = clientY - rect.top;
        nx = Math.max(0, Math.min(nx, rect.width)); ny = Math.max(0, Math.min(ny, rect.height));
    ...
    // --- 錬成ツール ---
    const hexToRgb = hex => { const bigint = parseInt(hex.slice(1), 16); return { r: (bigint>>16)&255, g: (bigint>>8)&255, b: bigint&255 }; };
    const clamp = val => Math.max(0, Math.min(255, Math.round(val)));
    const rgbToHex = (r, g, b) => `#${[r,g,b].map(x => x.toString(16).padStart(2, '0')).join('')}`;
    const hsvToRgb = (h, s, v) => {
        s/=100; v/=100; let c=v*s, x=c*(1-Math.abs((h/60)%2-1)), m=v-c; let r=0, g=0, b=0;
    ...
    document.getElementById("submit-btn").addEventListener("click", () => {
        ...
        document.getElementById("skip-btn").onclick = () => { skippedWait = true; clearTimeout(loadTimer); finalizeColor(); };
    });
    ...
    const finalizeColor = () => {
        ...
        selectedMotifs.forEach((m, i) => { let w = i===0?1.0:(i===1?0.6:0.3); baseR+=m.r*w; baseG+=m.g*w; baseB+=m.b*w; });

        const text = document.getElementById("mood-text").value.trim();
        let textLog = "標準";
        if (text === "わからない" || text === "不明" || text === "無" || text === "特になし") {
            baseR -= 30; baseG -= 10; baseB += 40; textLog = "感情の言語化放棄(Ti優勢) → 青色増幅";
        } else {
        ...
        baseR = (baseR*0.9) + (bColor.r*0.1); baseG = (baseG*0.9) + (bColor.g*0.1); baseB = (baseB*0.9) + (bColor.b*0.1);
        ...
        document.getElementById("main-color-box").style.background = mainHex; document.getElementById("main-hex").textContent = mainHex;
        document.getElementById("sub-color-box").style.background = subHex; document.getElementById("sub-hex").textContent = subHex;
        document.getElementById("accent-color-box").style.background = accHex; document.getElementById("accent-hex").textContent = accHex;
        ...
        // 🌟 GASへデータ送信 (JSONP方式：エラー回避)
        // ※エラーの原因だったHTMLタグ（<li>など）をすべて削除して綺麗なテキストにする処理を追加！
        ...
        window.gasCallback = function(data) {
            ...
            // ★ エラーが起きたら、詳細な理由を画面に出す！！
            if (data.error) {
                console.error("GASエラー詳細:", data.details);
                dbArea.innerHTML = `<span style="color:red;">※データベースエラー: ${data.details}</span>`;
                return;
            }
            
            let html = "";
            if (data.perfectMatch === 0) {
                html += `✨ 驚異的！全世界で<br><strong style="font-size:18px; color:#ff758c;">あなたが初めて</strong>この色を出しました。<br>`;
            } else {
                html += `過去に <strong>${data.perfectMatch}人</strong> が<br>全く同じ3色に辿り着いています。<br>`;
            }
            html += `<div style="font-size: 11px; margin-top: 5px; color: #777;">`;
            if (data.partialMatch > 0) {
                html += `(※3色のうち1色でも同じ色を出した人は過去に ${data.partialMatch}人 います)`;
            } else {
                html += `(※1色でも被った人は過去に誰もいません。完全なる唯一無二です！)`;
            }
            html += `</div>`;
    ...
    document.getElementById("save-btn").onclick = () => {
        ...
        html2canvas(target, { scale: 2, backgroundColor: "#ffffff" }).then(canvas => {
            ...
            if (!isMobile) {
                ...
                link.href = imgData;
                link.download = "chromalog-result.png";
                link.click();
                document.getElementById("save-btn").innerHTML = "<i class='fa-solid fa-check'></i> ダウンロード完了！";
            } else {
                document.getElementById("generated-image").src = imgData;
                document.getElementById("image-output").classList.remove("hidden");
                document.getElementById("save-btn").textContent = "画像生成完了！下へ↓";
            }
        });
    };
